...
public class g {
   ...
   */
  public static String get(String f, int x, String s) { return get(f + "." + x, s); }
  /**
  ...
  public static String get(String key, String sDefault) {
    ...
    if (key.length() > 1 && key.equals(key.toUpperCase())) {
    ...
    try {
      ...
      if (sr.length() > 6 && sr.substring(sr.length() - 6).equals("[auto]")) {
        sr = sr.substring(0, sr.length() - 6);
      } // on retir [auto] a la fin de la String si besoin
      else if (sr.length() > 2 && sr.substring(sr.length() - 2).equals("[]")) {
        sr = sr.substring(0, sr.length() - 2);
      } // on retire les simple crochet aussi.
      if (sr.equals("ø")) {
        return "";
      }
   ...
   * @lastEditedVersion 1.33
   */
  public static boolean exist(String key) { return getElementMap(key) != null && !getElementMap(key).equals(""); }
}
